...
public class GeradorJFrame {
    ...
    public static void main(String[] args) {
        if (args.length != 1) {
            JOptionPane.showMessageDialog(null, 
            		"Por favor, forneça o nome da classe como argumento.");
            return;
        }
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, 
            		"Classe não encontrada: " + nomeClasse);
            e.printStackTrace();
        }
    }
    ...
    public static void criarJFrame(Class<?> classe) {
        ...
        List<JComboBox> camposCombo = new ArrayList<>();
        ...
        for (Field campo : campos) {
            ...
            if (!Modifier.isStatic(campo.getModifiers()) && !Modifier.isFinal(campo.getModifiers())) {
            	...
            	if (campo.getType().equals(boolean.class)) {
            		...
            		JComboBox<String> comboOpcao = 
            				new JComboBox<String>(opcoes);
            		frame.add(label);
	                frame.add(comboOpcao);
	                camposCombo.add(comboOpcao);
            	}else if(campo.getType().isEnum()) {
            		JComboBox comboOpcao = 
            				new JComboBox(campo.getType().getEnumConstants());
            		frame.add(label);
	                frame.add(comboOpcao);
	                camposCombo.add(comboOpcao);
            	}else{
